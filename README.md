...
### 🎬 初阳-视频生成 Pro
- ✅ **文生视频**：根据文本提示生成视频
- ✅ **图生视频-首帧**：使用单张图片作为首帧生成视频
- ✅ **图生视频-首尾帧**：使用两张图片作为首尾帧生成视频
- ✅ 支持 **Doubao Seedance** 模型（Pro / Pro Fast）
- ✅ 支持 **即梦** 模型（v3.0 / v3.0 Pro）
- ✅ 即梦 v3.0 支持 **11种运镜模板**（希区柯克、机械臂、环绕等）
...
| `API密钥` | STRING | API 身份验证密钥 | sk-xxx |
| `API地址` | STRING | API 服务端点 | 默认：https://apitt.cozex.cn/v1/images/generations |
| `模型` | ENUM | 选择模型 | Seedream 4.0 / Seedream 4.5 |
| `尺寸` | ENUM | 图片分辨率 | 1K / 2K / 4K |
| `序列生成` | ENUM | 是否生成组图 | 关闭 / 自动 |
| `最大图片数` | INT | 组图模式下最多生成数量 | 1-15 |
| `响应格式` | ENUM | 返回格式 | URL / Base64 |
| `添加水印` | BOOLEAN | 是否添加水印 | true / false |
| `图片1-4` | IMAGE | 可选输入图片 | 用于图生图/多图融合 |
...
| 参数 | 类型 | 说明 | 可选值 |
|------|------|------|--------|
| `提示词` | STRING | 视频生成的文本描述 | 支持中英文 |
| `API密钥` | STRING | API 身份验证密钥 | sk-xxx |
| `API地址` | STRING | API 服务端点 | 默认：https://apitt.cozex.cn/v1/video/generations |
| `模型类型` | ENUM | 选择模型系列 | Doubao / 即梦 |
| `Doubao模型` | ENUM | Doubao 模型 | Seedance Pro / Pro Fast |
| `即梦模型` | ENUM | 即梦模型 | v3.0 / v3.0 Pro |
| `分辨率` | ENUM | 视频分辨率 | 480p / 720p / 1080p |
| `宽高比` | ENUM | 视频比例 | 16:9 / 9:16 / 1:1 等 |
| `时长` | INT | 视频时长（秒） | 2-12 |
| `帧率` | INT | 视频帧率 | 24 / 30 |
| `随机种子` | INT | 控制生成随机性 | -1=随机 |
| `首帧图片` | IMAGE | 可选首帧图片 | 图生视频 |
| `尾帧图片` | IMAGE | 可选尾帧图片 | 首尾帧模式 |
| `运镜模板` | ENUM | 即梦运镜效果 | 11种模板可选 |
| `运镜强度` | ENUM | 运镜幅度 | 弱 / 中 / 强 |

**运镜模板列表**（仅即梦 v3.0 的 720p 支持）：
- 希区柯克推进/拉远
- 机械臂
- 动感环绕 / 中心环绕
- 起重机
...
**功能模式**：
1. **文生视频**：只填提示词，不提供输入图片
2. **图生视频-首帧**：提供首帧图片
3. **图生视频-首尾帧**：提供首帧和尾帧图片（仅 Doubao Pro 和即梦 v3.0 支持）
4. **运镜视频**：选择运镜模板（仅即梦 v3.0 720p 支持）
...
- ✅ **新增节点**：初阳-图片生成 Pro
  ...
  - 详细的调试日志
- ✅ **新增节点**：初阳-视频生成 Pro
  - 支持 Doubao Seedance (Pro / Pro Fast) 模型
  - 支持即梦 v3.0 和 v3.0 Pro 模型
  - 文生视频、图生视频（首帧/首尾帧）
  - 即梦 v3.0 支持 11 种运镜模板
  - 支持 480p/720p/1080p 分辨率
  - 自动轮询任务状态
