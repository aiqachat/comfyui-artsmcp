...
**2. 必需参数**
- `提示词` (STRING): 视频生成的提示词描述
  - 支持多行输入：配合“启用提示词分行”功能使用
  - 详细描述场景、动作、镜头、氛围等
  - 示例：`"多个镜头。一名侦探进入一间光线昏暗的房间。他检查桌上的线索，手里拿起桌上的某个物品。镜头转向他正在思索。背景音乐低沉神秘。"`
- `API密钥` (STRING): API 密钥
- `API地址` (STRING): API 服务地址（例如：`https://api.cozex.cn`）
- `模型` (下拉框): 模型选择
...
*输入图片：*
- `参考图片1` (IMAGE): 第一张输入图片（图生视频的起始帧）
- `参考图片2` (IMAGE): 第二张输入图片（结束帧或中间帧）
...
- `时长` (INT): 视频时长，2-12秒（默认：5秒）
- `帧率` (下拉框): 帧率，仅支持24

*高级参数：*
- `随机种子` (INT): 种子整数，用于控制生成内容的随机性
  - -1 = 随机（会使用随机数替代）
  - [-1, 2^32-1] 之间的整数 = 固定值可生成类似结果（但不保证完全一致）
  - ℹ️ **重要**：seed 值不会自动变化，保持用户设置的值不变
- `固定镜头` (BOOLEAN): 是否固定摄像头（仅文生视频支持）
- `水印` (BOOLEAN): 是否添加水印（默认：False）
- `生成音频` (BOOLEAN): 是否生成包含画面同步音频的视频（默认：False）
  - ⭐ **新功能**：仅 Seedance 1.5 pro 支持
  - `True`: 生成带有画面同步音频的视频
  - `False`: 生成无声视频

*并发控制：*
- `并发请求数` (INT): 同时生成的视频数量，1-10（默认：1）
  - 设置 > 1 时自动启用并发模式
  - 配合“启用提示词分行”使用，否则重复生成相同提示词的视频
- `启用提示词分行` (BOOLEAN): 是否按行拆分提示词（默认：False）
  - 启用后：按换行符拆分提示词，每行一个任务
  - 未启用：根据并发请求数重复相同提示词

*系统参数：*
- `轮询间隔` (INT): 轮询间隔，2-30秒（默认：10秒）
- `超时秒数` (INT): API请求超时时间，60-600秒（默认：60秒）
- `最大等待时长` (INT): 最大轮询时间，60-3600秒（默认：300秒）
- `详细日志` (BOOLEAN): 调试模式，输出完整 API 响应（默认：False）
- `忽略SSL证书` (BOOLEAN): 忽略 SSL 证书校验（仅用于调试，不安全，默认：False）
  - ⚠️ **生产环境建议关闭**：默认开启 SSL 证书校验，仅在内网测试时才开启此选项
